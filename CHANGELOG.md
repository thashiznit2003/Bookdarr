...
-## 1.3.20
- Summary: Book Pool now converts each book’s covers to their cached `/MediaCover/Books/...` URLs without triggering new downloads, and the sidebar shows any throttle warning in the lower-left notification area.
- Why: Rendering dozens of remote thumbnails at once had repeatedly hit provider throttling and left most posters blank, and there was no persistent signal when HTTP requests were being rate limited.
- Impact: `UserLibraryController.GetBookPool()` converts every book’s images to the local cover URLs, `HttpClient` records 429s via a new `IHttpThrottleNotificationService` that surfaces `/system/throttle`, and the sidebar renders a throttling alert above the diagnostics footer so rate limits are obvious while the cached covers keep the grid full. The new API and localization entries keep the message aligned with the existing diagnostics area.
...
- Next: tag and push `snapshot-YYYYMMDD-HHMM`, then rerun `LOG_FILE="/opt/bookdarr-dev/Logs/update-093.log" /opt/bookdarr-dev/scripts/update-dev.sh` over SSH so diagnostics show v1.3.20 and the new throttle warning alongside the cached covers.
