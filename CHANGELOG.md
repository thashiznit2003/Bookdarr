...
- Impact: Manual import requests from an upload folder now return all files for selection.
- Files: `frontend/src/Store/Actions/interactiveImportActions.js`, `src/Directory.Build.props`, `CHANGELOG.md`
- Next: Tag `snapshot-YYYYMMDD-HHMM` and `v2.1.73`, push, then run the SSH update with the next log number.
...
- Impact: If the target file exists, Bookdarr links or updates the existing entry and completes the import successfully.
- Files: `src/NzbDrone.Core/MediaFiles/BookImport/ImportApprovedBooks.cs`, `src/Directory.Build.props`, `CHANGELOG.md`
- Next: Tag `snapshot-YYYYMMDD-HHMM` and `v2.1.57`, push, then run the SSH update with the next log number.

## 2.1.56
- Summary: Fix build after author-search fallback normalization.
- Why: Used the wrong Parser type in the fallback matcher, causing a compile error.
- Impact: Build succeeds and author-name fallback remains in place.
- Files: `src/Readarr.Api.V1/Author/AuthorBooksController.cs`, `src/Directory.Build.props`, `CHANGELOG.md`
- Next: Tag `snapshot-YYYYMMDD-HHMM` and `v2.1.56`, push, then run the SSH update with the next log number.
...
- Impact: Author Available Books now returns an empty list when rate-limited instead of a hard error.
- Files: `src/Readarr.Api.V1/Author/AuthorBooksController.cs`, `src/Directory.Build.props`, `CHANGELOG.md`
- Next: Tag `snapshot-YYYYMMDD-HHMM` and `v2.1.48`, push, then run the SSH update with the next log number.
...
- Impact: Author available-books filtering builds successfully again.
- Files: `src/Readarr.Api.V1/Author/AuthorBooksController.cs`, `src/Directory.Build.props`, `CHANGELOG.md`
- Next: Tag `snapshot-YYYYMMDD-HHMM` and `v2.1.29`, push, then run the SSH update with the next log number.
