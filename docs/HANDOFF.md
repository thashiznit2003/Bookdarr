...
- Update commands should `tee` to `/opt/bookdarr-dev/Logs/update-0XX.log`, next up is `update-03.log`.
...
- After pushing to GitHub, immediately run the SSH update workflow (`ssh -i ~/.ssh/bookdarr-agent joe@192.168.0.103 'sudo /opt/bookdarr-dev/scripts/update-dev.sh 2>&1 | sudo tee -a /opt/bookdarr-dev/Logs/update-03.log'`) so the Ubuntu VM mirrors the latest version and the diagnostics bundle is generated.
...
```
sudo /opt/bookdarr-dev/scripts/update-dev.sh 2>&1 | sudo tee -a /opt/bookdarr-dev/Logs/update-03.log
```
