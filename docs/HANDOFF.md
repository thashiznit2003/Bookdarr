...
- After pushing to GitHub, immediately run the SSH update workflow (`ssh -i ~/.ssh/bookdarr-agent joe@192.168.0.103 'sudo /opt/bookdarr-dev/scripts/update-dev.sh 2>&1 | sudo tee -a /opt/bookdarr-dev/Logs/update-172.log'`) so the Ubuntu VM mirrors the latest version and the diagnostics bundle is generated.
...
- Combine audiobooks now supports FLAC sources (transcodes when needed) instead of erroring; version bumped to 1.3.107.
